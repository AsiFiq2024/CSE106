...
private:
    ...
    void printItemStats(Node *rt) const
    {
        if (!rt)
            return;
        printItemStats(rt->left);
        if constexpr (std::is_class<Value>::value)
        {
            cout << "  " << rt->key << ": "
                 << "Current bid: " << rt->value.getCurBid() << ", "
                 << "Total bids: " << rt->value.getTotalBid() << ", "
                 << "Successful: " << rt->value.getSuccessBid() << ", "
                 << "Rejected: " << rt->value.getRejectBid() << "\n";
        }
        else
        {
            cout << "  " << rt->key << ": " << rt->value << "\n";
        }
        printItemStats(rt->right);
    }
    void printItemInO(Node *rt) const
    {
        if (rt == nullptr)
        {
            return;
        ...
        printItemInO(rt->left);
        if constexpr (std::is_class<Value>::value)
        {
            cout << "(" << rt->key << ":" << rt->value.getCurBid() << ") ";
        }
        else
        {
            cout << "(" << rt->key << ":" << rt->value << ") "; // Changed this line
        }
        printItemInO(rt->right);
    }
